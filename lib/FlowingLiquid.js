export default class FL {
  ...
  frameRender (xOffset) {
    ...
    for (let x = startX; x < pointXLimit; x += 20 / canvasWidth) {
      ...
      points.push({
        x,
        y: (1 - this.currentLine) * this.canvasHeight + y
      })
      context.lineTo(x, (1 - this.currentLine) * this.canvasHeight + y)
    }
  ...
  animator () {
    ...
    if (this.currentLine < this.waterline) this.currentLine += 0.01
